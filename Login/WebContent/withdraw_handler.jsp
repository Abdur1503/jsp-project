...
<%
   int id=Integer.parseInt(request.getParameter("i"));
	...
	Connection cref=DriverManager.getConnection(url, user, password);
		...
		while(rs.next())
		{
			if(id==rs.getInt(1))
			{
			   	 ...
			   	 if(wamnt<=amount)
			   	 {
			   	  ...
			   	  %>
			   	 <b>Succesful withdraw</b><br><a href="welcome.jsp?i=<%=id%>&em=<%=em%>">click here to go home</a>
		...
		if(z==0)
		 {%>
			<b>Invalid amount</b><br><a href="withdraw.jsp?i=<%=id%>">click here to withdraw</a>
<%} %>
</body>
</html>